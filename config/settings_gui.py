...
class HyprConfGUI(Window):
    ...
    def create_system_tab(self):
        ...
        if self.show_lock_checkbox:
            ...
            self.lock_switch = Gtk.Switch(
                tooltip_text="Replace Hyprlock configuration with Rz-Shell's custom config"
            )
        ...
        if self.show_idle_checkbox:
            ...
            self.idle_switch = Gtk.Switch(
                tooltip_text="Replace Hypridle configuration with Rz-Shell's custom config"
            )
    ...
    def create_about_tab(self):
        ...
        repo_link = Label(
            markup='<a href="https://github.com/rzzdr/Rz-Shell">https://github.com/rzzdr/Rz-Shell</a>'
        )
    ...
    def on_accept(self, widget):
        ...
        def _apply_and_reload_task_thread(user_data):
            ...
            print(f"{time.time():.4f}: Initiating Rz-Shell restart using Popen...")
            ...
            print(f"{time.time():.4f}: Rz-Shell restart commands issued via Popen.")
            end_time = time.time()
            print(
