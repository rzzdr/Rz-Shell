...
if [ -d "$INSTALL_DIR" ]; then
    echo "Updating Rz-Shell..."
    git -C "$INSTALL_DIR" pull
else
    echo "Cloning Rz-Shell..."
...
/usr/bin/python3.13 "$INSTALL_DIR/config/config.py"
echo "Starting Rz-Shell..."
killall rz-shell 2>/dev/null || true
uwsm app -- /usr/bin/python3.13 "$INSTALL_DIR/main.py" > /dev/null 2>&1 & disown
